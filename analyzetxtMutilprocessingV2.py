...
def searchingUnderInputDir(imagedir):
    ...
    filelist.extend(glob.glob(os.path.join(imagedir+'/', '*.JPG')))
    filelist.extend(glob.glob(os.path.join(imagedir+'/', '*.tiff')))
    for v in filelist:
        try:
...
def FileList(pathlist, pp, tag):
    ...
    fsum='%s_summary.txt'%(tag)
    fin=open(fsum,'w')
    fin.write('Total: %d\nUndetected: %d\nRatio: %f\n'%(samples_count, undetect_count, undetect_count/samples_count))
    fin.close()
    ferr='%s_nofacedetected.txt'%(tag)
    fin=open(ferr,'w')
    for v in nl:
        fin.write('%s\n'%(v))
    fin.close()
    flabel='%s_label.txt'%(tag)
    fin=open(flabel,'w')
    for v in fl:
        fin.write(v)
    fin.close()
    flabel='%s_label_random.txt'%(tag)
    fin=open(flabel,'w')
    random.shuffle(fl)
    for v in fl:
        fin.write(v)
...
if __name__=='__main__':
    ...
    if len(sys.argv)==2:
        ...
        Pn=int(sys.argv[1])
        imagedir=imagedir.extend(str(sys.argv[2]).split(','))
    else:
        Pn=7

    pp=pool.Pool(processes=Pn)
    FileList(imagedir, pp, tag)#only need to implement once
    t1=time.time()
